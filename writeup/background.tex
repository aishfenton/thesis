...
\picscl{images/vrp.pdf}{An example of customers being assigned to three different vehicle routes. The depot is the black dot in the centre.}{fig:vrp_example}{0.55}
...
\picscl{images/tsp.pdf}{Shown is an example of a 16 city \TSP\ tour. This tour is one of 20,922,789,888,000 possible tours for these 16 cities.}{fig:tsp}{0.60}
...
A Dynamic Programming approach was first applied to the \VRP\ by Eilon, Watson-Gandy and Christofides~\cite{EWC:1971}. Their approach allowed them to solve exactly for problems of 10 to 25 customers. Since then, Christofides has made improvements to this algorithm to solve exactly for problems up to fifty customers.
...
In general, operations such as 3-Opt, that remove edges and then search for a more optimal recombination of components take \BIGO{n^y} where $y$ is the number of edges removed. A profitable strain of research has focused on producing operations that reduce the amount of recombinations that must be searched. Or presents an operation that has since come to be known as Or-Opt~\cite{Or:1976}. Or-Opt is a restricted 3-Opt. It searches for a relocation of all sets of 3 consecutive vertices (which Or calls chains), such that an improvement is made. If an improvement cannot be made then it tries again with chains of 2 consecutive vertices, and so on. Or-Opt has been shown to produce similar results to that of 3-Opt, but with a running time of \BIGO{n^2}. More recently Renaud, Boctor, and Laptorte~\cite{RBL:1996} have presented a restricted version of 4-Opt, called 4-Opt*, that operates in a similar vein to Or-Opt. 4-Opt* has a running time of \BIGO{wn^2} where $w$ denotes the number of edges spanned by 4-Opt* when building a chain. 
...
Where $C^k$ represents the total distance travelled through the graph by ant $k$. This ensures that shorter paths result in more pheromone being deposited.
...
Where $s$ represents the proximity of customers $i, j$ to the depot, and $\kappa = (Q^i + q^j) / Q$---$Q$, giving the maximum capacity, $Q^i$ giving the capacity already used on the vehicle, and $q^j$ is the additional load to be added. $\kappa$ influences the ants to take advantage of the available capacity. 
...
\begin{algorithm}[H]
   ...
   \While{termination condition not met}{
      sortByFitness($B$) \\
      $E = \set{b_1, b_2,\ldots,b_e}$ \\
      $R = \set{b_{e+1}, b_{e+2},\ldots,b_{m}}$ \\
      searchNeighbourhood($E \union \set{c_1,\ldots,c_{nep}}$) \\
      searchNeighbourhood($R \union \set{d_1,\ldots,d_{nsp}}$) \\
